...
class newcorpusayahWordArrayListrefFlowAyahWordAdapter(
    isaudio: Boolean,
    ...
    private val ayahWordArrayList: List<QuranCorpusWbw>?,
    var context: Context,
    val surah_id: Long,
    private val SurahName: String,
    private val isMakkiMadani: Int,
    listener: OnItemClickListenerOnLong?,
) : RecyclerView.Adapter<newcorpusayahWordArrayListrefFlowAyahWordAdapter.ItemViewAdapter>() //implements OnItemClickListenerOnLong {
{
    ...
    private lateinit var ayahWord: QuranCorpusWbw
    ...
    override fun getItemId(position: Int): Long {
        ...
        itemId = ayahWord.corpus.ayah.toLong()
    ...
    private fun wordBywordWithTranslation(
        ...
        showWordColor: Boolean,
        wbw: String?,
        ayahWord: QuranCorpusWbw,
        ayahWordArrayList: List<QuranCorpusWbw>,
        ...
        for (word in ayahWordArrayList) {
            var aindex = 0;

            val wordcount = ayahWordArrayList.get(0).corpus.wordcount
            ...
            if (showrootkey) {
                /*        spannedroot = if (!word.rootword!!.isEmpty()) {
                            ...
                            SpannableString.valueOf(word.rootwords)
                        }*/
            ...
            if (showWordColor) {
                ...
                arabicChipview.setText(rootwords)
            }
            rootword.setText(spannedroot)
            ...
            if (!showWbwTranslation) {
                arabicChipview.setOnLongClickListener(View.OnLongClickListener { v ->
                    ...
                    val verbCorpusRootWords =
                        utils.getQuranRoot(word.corpus.surah, word.corpus.ayah, word.corpus.wordno)
                    if (verbCorpusRootWords!!.size > 0 && verbCorpusRootWords[0]!!.tag == "V") {
                        //    vbdetail = ams.getVerbDetails();
                        print("check")
                    }
                    val corpusNounWord =
                        utils.getQuranNouns(word.corpus.surah, word.corpus.ayah, word.corpus.wordno)
    ...
    private fun setChapterInfo(holder: ItemViewAdapter, verse: QuranCorpusWbw) {
        val surahInfo = java.lang.StringBuilder()
        //        surahInfo.append(surahName+".");
        surahInfo.append(verse!!.corpus.surah).append(".")
        surahInfo.append(verse.corpus.ayah).append("-")
        surahInfo.append(SurahName)
        val sharedPreferences = androidx.preference.PreferenceManager.getDefaultSharedPreferences(
            context
        )
                ...
                fabmenu.setOnClickListener(object : View.OnClickListener {
                    ...
                    private fun showFABMenu() {
                        ...
                        tafsir.setOnClickListener(View.OnClickListener { view12: View? ->
                            ...
                            val verse = ayahWordArrayList!![position - 1].corpus!!.ayah
                    ...
                    private fun bookMarkSelected(position: Int) {
                        val chapter_no = ayahWord!!.corpus.surah
                        //   int verse = ayahWord.getWord().get(0).getVerseId();
                        val verse = ayahWordArrayList!![getPosition() - 1].corpus.ayah
                        val en = BookMarks()
